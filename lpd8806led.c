...
void write_frame (lpd8806_color * p, uint8_t red, uint8_t green,
		  uint8_t blue);
...
int
lpd8806_init (lpd8806_buffer * buf, int leds)
{
  buf->leds = leds;
...
void
write_color (lpd8806_color * p, uint8_t red, uint8_t green, uint8_t blue)
{
  // uint8_t flag;
...
int
send_buffer (int filedes, lpd8806_buffer * buf)
{
  int ret;
...
void
lpd8806_free (lpd8806_buffer * buf)
{
  free (buf->buffer);
  buf->buffer = NULL;
  buf->pixels = NULL;
...
void
write_frame (lpd8806_color * p, uint8_t red, uint8_t green, uint8_t blue)
{
  // p->flag=flag;
  p->blue = blue;
  p->green = green;
...
uint8_t
make_flag (uint8_t red, uint8_t green, uint8_t blue)
{
  uint8_t flag;
...
ssize_t
write_all (int filedes, const void *buf, size_t size)
{
...
void
set_gamma (double gamma_red, double gamma_green, double gamma_blue)
{
  int i;
...
void
write_gamma_color (lpd8806_color * p, uint8_t red, uint8_t green,
		   uint8_t blue)
{
  uint8_t flag;
  ...
  write_frame (p, gamma_corrected_red, gamma_corrected_green,
